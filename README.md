...
| `data/downloads/` | Cowrieが実際に保存した取得物 | マルウェア疑いのファイルを含む可能性があるため、開かない、実行しない、Gitへコミットしない |
| `data/public/` | `summary.csv` などの分析結果 | 公開用に匿名化した生成物でも、検証用ファイルとしてGit管理しない |

`wget` や `curl` などの入力コマンドは `logs/cowrie/` に記録されます。`data/downloads/` はそのコマンドから推測した一覧ではなく、Cowrieがファイルとして保存した取得物の置き場です。
