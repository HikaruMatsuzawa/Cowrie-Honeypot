...
| `data/downloads/` | 攻撃者が偽シェル内で取得しようとしたファイル、またはCowrieが保存した取得物 | マルウェア疑いのファイルを含む可能性があるため、開かない、実行しない、Gitへコミットしない |
...
9. ホスト側firewallでCowrieコンテナの外向き通信を遮断する。
10. 22番と23番がCowrieへ到達し、管理用OpenSSHが22番で待ち受けていないことを確認する。
11. Cowrieログの `src_ip` に実送信元IPが記録されることを確認する。
12. 外向き通信制限、ログ永続化、停止・復旧手順を確認する。
