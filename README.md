...
AWS Lightsail上でCowrieを安全に運用し、SSH/Telnetハニーポットのログを収集・正規化・集計するための学習用プロジェクトです。
...
- ローカルではCowrieを外部公開せず、SSHは `127.0.0.1:2222`、Telnetは `127.0.0.1:2223` だけで確認します。
- Lightsailでは管理用OpenSSHを22番以外へ移動し、インターネットからのTCP 22番をCowrieへ割り当てます。
- LightsailではTCP 23番もCowrie Telnetへ割り当てます。
- Lightsailでは実送信元IPを記録するため、Cowrieコンテナ自身がホスト側TCP 22番を受けます。
- Cowrieコンテナは固定サブネットのDockerネットワークに置き、Lightsailではホスト側firewallにより外向き通信を遮断します。
...
どちらも `TcpTestSucceeded : True` が表示されれば正常です。
...
Cowrieの運用で作成される主なデータは次の通りです。

| パス | 内容 | 注意 |
| --- | --- | --- |
| `logs/cowrie/` | Cowrieの生ログ、JSONログ、TTYログ | 実IP、入力ユーザー名、入力パスワード、入力コマンドを含むためGitへコミットしない |
| `data/downloads/` | Cowrieが実際に保存したファイル転送・取得物 | マルウェア疑いのファイルを含む可能性があるため、開かない、実行しない、Gitへコミットしない |
| `data/public/` | `summary.csv` などの分析結果 | 公開用に匿名化した生成物でも、検証用ファイルとしてGit管理しない |

`wget` や `curl` などの入力コマンドは `logs/cowrie/` に記録されます。`data/downloads/` はそのコマンドから推測した一覧ではなく、Cowrieがファイルとして保存したものの置き場です。このプロジェクトではCowrieコンテナの外向き通信を遮断するため、外部URLからのダウンロードは失敗し、保存されない想定です。

SSHやTelnetの動作確認で自分が接続したログも、攻撃者のログと同じ `logs/cowrie/` に記録されます。本番観測へ切り替える前に検証ログを消したい場合は、[運用手順の検証データのリセット](docs/deployment/operations.md#検証データのリセット) を実施します。
...
3. Lightsailファイアウォールで管理用SSH用の一時ルールを作る。
4. 管理用OpenSSHを22番以外へ移動し、管理者IPからのみ接続できるようにする。
5. Lightsailファイアウォールを最終形にし、Cowrie用SSH 22番とTelnet 23番を同時に設定する。
6. Docker EngineとCompose pluginをインストールする。
7. GitHubからこのリポジトリをcloneする。
8. `.env.lightsail.example` を参考に `.env` を作成する。
9. `compose.yaml` を使ってCowrieを起動する。
10. ホスト側firewallでCowrieコンテナの外向き通信を遮断する。
11. 22番と23番がCowrieへ到達し、管理用OpenSSHが22番で待ち受けていないことを確認する。
12. Cowrieログの `src_ip` に実送信元IPが記録されることを確認する。
13. 外向き通信制限、ログ永続化、停止・復旧手順を確認する。
