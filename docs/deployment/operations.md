...
- `cowrie` が起動している。
- `cowrie` だけが `127.0.0.1:2222` と `127.0.0.1:2223` を公開している。
...
- 2222番と2223番の両方で `TcpTestSucceeded : True` が表示される。
- Cowrieのシェル応答が表示される。
...
OpenSSHを22番から移動する前は、22番をいきなりCowrie用として全体公開しない。まず管理用OpenSSHへ入れる状態を保ったまま、移動先の管理用ポートを追加する。

一時ルール例:

| 用途 | プロトコル | ポート | 接続元 |
| --- | --- | --- | --- |
| 初期管理用OpenSSH | TCP | 22 | 管理者IP/32 |
| 新管理用OpenSSH | TCP | 22222 | 管理者IP/32 |
| HTTP/HTTPS | TCP | 80/443 | 開けない |
| その他 | 任意 | 任意 | 開けない |

管理用OpenSSHが22222番で接続でき、22番で待ち受けていないことを確認した後、最終形へ変更する。最終形ではCowrie用SSH 22番とCowrie用Telnet 23番を同時に設定する。
...
| Cowrie観測用Telnet | TCP | 23 | 任意のIPv4 |
...
- Cowrie用SSH 22番とCowrie用Telnet 23番は、管理用OpenSSHを22222番へ移してから公開する。
...
- `2222/tcp -> 0.0.0.0:22` が表示される。
- `2223/tcp -> 0.0.0.0:23` が表示される。
- Telnet待ち受け確認で `exit_code=0` が表示される。
...
`data/public` へ書き込めない場合は、`sudo mkdir -p data/public` で作成し、分析コマンドを実行するユーザーが書き込めるように `sudo chown -R "$USER:$USER" data/public` を実行する。
...
過去のログを消して、新しい観測データだけで確認し直す場合に使う。SSHやTelnetの動作確認で自分が接続したログを消し、以後の攻撃者由来のログだけを収集したい場合も、この手順を実施する。
